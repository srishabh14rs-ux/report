...
*&  Optimized: direct SELECT DISTINCT on @gt_doc_keys/@gt_output.
*&
*&  Performance improvements:
*&  1. Removed driver-table build loops (lt_bukrs, lt_bk_keys, lt_prctr).
*&  2. SELECT DISTINCT joined directly on @gt_doc_keys/@gt_output.
*&  3. lt_resolved: HASHED pre-resolved map (O(1) lookup).
*&  4. Master-data reads use BINARY SEARCH (O log L).
*&  5. VALUE # FOR fills lt_resolved in one shot (no LOOP/INSERT).
*&---------------------------------------------------------------------*
  METHOD m_enrich_with_master_data.
...
*======================================================================*
*  STEP 1 - Prepare gt_doc_keys: sort, dedup, build lt_resolved
*  PC mode: DESCENDING prctr (non-blank wins)
*  IO mode: DESCENDING aufnr (non-blank wins)
*  VALUE # FOR fills lt_resolved in one expression (safe after dedup).
*======================================================================*
    IF p_rdpc = abap_true.
...
*======================================================================*
*  STEP 2 - Master-data SELECTs with direct JOINs on internal tables
*  SELECT DISTINCT to avoid duplicates from multiple doc keys.
*======================================================================*
    ...
    IF p_rdpc = abap_true.
      SELECT DISTINCT k~kunnr, k~name1, k~ktokd, a~city1, t~landx50
        FROM @gt_doc_keys AS d
    ...
    SELECT DISTINCT c~prctr, c~ltext
      FROM @gt_doc_keys AS d
    ...
    IF p_rdpc = abap_true.
      SELECT DISTINCT k~bukrs, k~kunnr, t~ztag1
        FROM @gt_doc_keys AS d
             INNER JOIN knb1 AS k ON  k~bukrs = d~rbukrs
                                  AND k~kunnr = d~kunnr
                                  AND d~kunnr <> @space
      ...
      SELECT DISTINCT k~bukrs, k~kunnr, t~ztag1
        FROM @gt_output AS o
             INNER JOIN knb1 AS k ON  k~bukrs = o~bukrs
                                  AND k~kunnr = o~kunnr
                                  AND o~kunnr <> @space
             LEFT OUTER JOIN t052 AS t ON t~zterm = k~zterm
        APPENDING TABLE @lt_pmt.
    ELSE.
      SELECT DISTINCT k~bukrs, k~kunnr, t~ztag1
        FROM @gt_output AS o
             INNER JOIN knb1 AS k ON  k~bukrs = o~bukrs
...
*======================================================================*
*  STEP 3 - Merge: one pass over gt_output -> gt_final
*  Single O(1) hash read replaces the old O(K) inner LOOP.
*  All master-data reads use BINARY SEARCH (O log L).
*======================================================================*
    CLEAR gt_final.
...
*-- O(1) hash lookup - resolves kunnr/prctr/aufnr ---------------*
